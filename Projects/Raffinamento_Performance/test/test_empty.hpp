...
TEST(Testmassimo, TestmassimoTriangle)
{
    ...
    unsigned int id=0,p =0;
    massimoElementoAttivo(vector,id,p);
    EXPECT_EQ(id, 30);
}
...
TEST(TestSplit2, TestSplit2)
{
    ...
    unsigned int k=0;
    split2(Triangles, Edges, Vertices, Triangles.size()-1,tempId, k, permissible, tempId1);
...
TEST(TestSplit2again, TestSplit2again)
{
    ...
    unsigned int k=0;
    split2(Triangles, Edges, Vertices, Triangles.size()-2, tempId, k, permissible, tempId1);
